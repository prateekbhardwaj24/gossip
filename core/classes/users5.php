...
public function checkInput($variable){
    ...
    public function checkEmail($email_mobile){
        $stmt = $this->pdo->prepare("SELECT email FROM my_users1 WHERE email = :email");
...
$sql = "INSERT INTO {$table}({$columns})VALUES ({$values})";
    ...
    public function userIdByUsername($username){
        $stmt = $this->pdo->prepare('SELECT user_id FROM my_users1 WHERE userLink = :username');
    ...
    public function userData($profileId){
       $stmt = $this->pdo->prepare("SELECT * FROM my_users1 LEFT JOIN profile ON my_users1.user_id = profile.userId WHERE my_users1.user_id = :user_id");

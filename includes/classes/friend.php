...
class Friend{
    ...
    public function make_pending_friends($my_id, $user_id){
        ...
        try{
            ...
            $stmt = $this->db->prepare($sql);
            $stmt->execute([$my_id, $user_id]);
            header('Location: friendpage.php?id='.$user_id);
            exit;
        }
        catch (PDOException $e) {
            die($e->getMessage());
    ...
    public function request_notification($my_id, $send_data){
        try{
            $sql = "SELECT sender, user_name, user_country, user_gender FROM `friend_request` JOIN my_users1 ON friend_request.sender = my_users1.user_id WHERE receiver = ?";
                ...
                if($send_data){
                    ...
                    foreach($all_users as $row){
                        if($row->user_one == $my_id){
                            $get_user = "SELECT user_id, user_name,lastseen,user_profile,user_gender FROM `my_users1`  WHERE user_id = ?";
...
//		try{
//			$sql = "select  count(*) from user_chating1, my_users1 where user_chating1.status= 0 and user_chating1.receiver=:user_id and my_users1.user_id=user_chating1.sender";
//                        $stmt = $this->db->prepare($sql);
